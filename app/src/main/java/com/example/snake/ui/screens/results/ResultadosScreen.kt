...
fun ResultadosScreen(
    ...
    var textoLog by rememberSaveable(claveLog) {
    ...
    Box(
        ...
        Column(
            ...
            TarjetaEstadoResultado(uiState)
            ...
            OutlinedButton(
                ...
                border = androidx.compose.foundation.BorderStroke(
                    1.dp,
                    SnakeLightGreen.copy(alpha = 0.4f)
                )
            ) {
                Text(
                    text = "← Nova partida",
...
@Composable
private fun TarjetaEstadoResultado(uiState: GameUiState) {
    val resultado = uiState.log?.resultado

    val (titulo, color) = when (resultado) {
        ResultadoPartida.GANADA ->
            "Has guanyat 🎉" to SnakeGreen
        ResultadoPartida.PERDIDA_COLISION ->
            "Has perdut per col·lisió" to BtnDanger
        ResultadoPartida.PERDIDA_TIEMPO ->
            "Temps esgotat ⏱" to BtnError
        null ->
            "Resultat no disponible" to SnakeLightGreen
    }
